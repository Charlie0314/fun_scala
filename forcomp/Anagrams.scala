...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(char => char).map(occ => (occ._1,occ._2.size)).toList.sortWith((p1,p2) => p1._1 < p2._1)
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
	dictionary.map(word => (wordOccurrences(word),word)).groupBy(p => p._1).map(pair => (pair._1,pair._2.map(p => p._2)))
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y.foldLeft(x.toMap)((m,p) => m.updated(p._1, m.getOrElse(p._1,0) - p._2)).filter(p => p._2 != 0).toList.sorted
